...
func TestRemoval(t *testing.T) {
	// These codes should all be the same "Aca" after removing b/B.
	codes := []string{"BAca", "bAca", "BAbca", "AcbBa", "AcaBb", "bAbcbab"}
	for _, code := range codes {
		result := removeFromPolymer(readCode(strings.NewReader(code)), 1)
		success := (compareUnits(result, 0, false) &&
			compareUnits(result.next, 2, true) &&
			compareUnits(result.next.next, 0, true) &&
			result.next.previous == result &&
